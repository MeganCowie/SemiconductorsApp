...
def fig0_surface(slider_Vg, slider_zins, slider_Eg, slider_epsilonsem, slider_WFmet, slider_EAsem, slider_donor, slider_acceptor, slider_emass, slider_hmass, slider_T, slider_alpha, slider_biassteps, slider_zinssteps):
    ...
    Vs_biasarray_top,F_biasarray_top,Es_biasarray_top,Qs_biasarray_top,P_biasarray_top = Organization_BuildArrays.Surface_biasarrays(Vg_array,zins,Na,Nd,epsilon_sem,T,CPD,LD,no,po,ni)
    #########################################################
    ...
    zgap,Vgap, zvac,Vvac, zmet,Vmet, zarray,Earray,Qarray  = Physics_BandDiagram.BandDiagram(Vg,zins,T,Nd,Na,WFmet,EAsem,epsilon_sem, ni,no,po,Vs,Ec,Ev,Ef,CPD, zsem,Vsem,Esem,Qsem)
    ...
    fig0.update_xaxes(title_standoff=5, title_text= "Gate Bias (eV)",row=7,col=2)# range=[Vg_array[biasrange_indexmin]/Physics_Semiconductors.e, Vg_array[biasrange_indexmax]]/Physics_Semiconductors.e)
    ...
    fig0.update_yaxes(title_standoff=5, title_text="log(|Qs|)", row=7, col=1)
