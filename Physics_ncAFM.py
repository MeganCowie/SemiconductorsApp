...
def zinslag_AFMarray(time_AFMarray,amplitude,frequency,lag,sampletype,  Vg,zins,Eg,epsilon_sem,WFmet,EAsem,Nd,Na,mn,mp,T):
    ...
    #lag_soln = Vsdiff_soln
    #lag_soln = lag*100*(Vsdiff_soln**2) #radians
    lag_soln = lag #radians
...
def dfdg(time_AFMarray,F_AFMarray,frequency,springconst,amplitude,Qfactor,tipradius):
    ...
    tiparea = np.pi*tipradius**2 #nm^2
    tipradius = tipradius*10**-9 #m
    ...
    E_o = np.pi*springconst*tipradius**2/Qfactor/Physics_Semiconductors.e*1000
    A_exc = dg
    A_exco = dg_addedterm
